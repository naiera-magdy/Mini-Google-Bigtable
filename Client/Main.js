$(document).ready(function () {
    ...
    $("#submitbtn").click(function (e) {
        ...
        if (radioValue === "setrow") {
            ...
            let socket = (rowkey >= Tabletcache[0].start && rowkey <= Tabletcache[0].end) ? TabletServer1 : TabletServer2;
        ...
        // Delete Rows
            ...
            let tabletArray1, tabletArray2 = [];
            $("#deleterow .rowkey").each(function (el) {
                if ($(this).val() >= Tabletcache[0].start && rowkey <= Tabletcache[0].end) {
            ...
            if (tabletArray1.length()) TabletServer1.emit("show:DeleteRow", tabletArray1);
            if (tabletArray2.length()) TabletServer2.emit("show:DeleteRow", tabletArray2);
        ...
        $("#main").html(`${JSON.stringify(objectToSend)} <div id="response"> </div> <button class="btn btn-primary mt-3" onclick="window.location.reload(true)">Restart</button>`);
